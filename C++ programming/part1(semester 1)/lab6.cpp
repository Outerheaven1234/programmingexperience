// Lucien Parker, Charlie Mirabile, Daniel Doherty, Vibishan Wigneswaran
// 10/29/2019
// Lab 6 COMP3Lab
#undef debug
...
#endif
    if(data<_data)
        if(_left)_left->insert(data);
        else addLeft(data);
    else
        if(_right)_right->insert(data);
        else addRight(data);
}
 ...
 */
 template<typename T>
 void BSTNode<T>::preOrderDisplay(ostream& out) const { //PLR
