...
int main(void) {
...

	{
		...
		cout << "Using std::find:\n";
		vector<UMLPerson>::iterator temp;
		// *** You write it!
		cout << "IDNumber 87654321 ";
		if ((temp = find(upDB.begin(), upDB.end(), 87654321)) != upDB.end())
		{
			cout << "found in upDB for UMLPerson: " << *temp << endl;
		}
		else
		{
			cout << "not found in upDB" << endl;
		}
		// *** You write it!
		cout << "IDNumber 89283002 ";
		if ((temp = find(upDB.begin(), upDB.end(), 89283002)) != upDB.end())
		{
			cout << "found in upDB for UMLPerson: " << *temp << endl;
		}
		else
...

	{
		...
		// *** You write it!
		auto result = minmax_element(upDB.begin(), upDB.end());
		...
		// *** You write it!
		result = minmax_element(upDB.begin(), upDB.end(), compare_IDs);
		cout << "min is " << *result.first << endl;
		cout << "max is " << *result.second << endl;
	}
