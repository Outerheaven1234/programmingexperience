...
public:
  //constructor/destructor
    ...
    PerCourse(unsigned percourse) : _feePerCourse(percourse) {}
    ~PerCourse() { cout << "PerCourse destructor called\n"; }

  unsigned int getfeePerCourse(){return _feePerCourse;}
  void setPerCourse(unsigned newVal) { _feePerCourse=newVal;}
private:
...
class PartTime : public EmployeeKind{
  //constructor/destructor
    PartTime() : __maxHours(0) {}
    PartTime(unsigned parttime) : __maxHours(parttime) {}
    ~PartTime() { cout << "PartTime destructor called\n"; }

public:
  unsigned int getmaxHours(){return __maxHours;}
  void setmaxHours(unsigned int hours){ __maxHours=hours;}
private:
  unsigned __maxHours;
};
...
class Adjunct : public Faculty, public PerCourse, public PartTime {
  ...
  //constructor/destructor
    ...
    Adjunct(unsigned _feePerCourse, unsigned _maxHours, string firstName, string lastName, string address, unsigned _IDNumber, string SSN, string department, unsigned salary);
    ...
    void output(ostream& out) const { out << *this; }
		...
		// accessor functions
		unsigned int getMaxCourses() const { return _maxCourses; }

	friend ostream& operator<< (ostream&, const Adjunct&);


  unsigned int getmaxCourses(){return _maxCourses;}
  void setmaxCourses(unsigned int courses){ _maxCourses=courses;}
...
ostream& operator<< (ostream&, const Adjunct&){
...
class Grader : public Undergrad, public Hourly, public PartTime{ // derived class
    public:
        ...
        Grader(string firstName, string lastName, string address, unsigned _IDNumber,
               double GPA, string smajor, string advisorName, string classRank,
			   string classNumber, unsigned hourlyRate, unsigned int _maxHours);
